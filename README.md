...
# Mentoring Session and Implementation Updates (March 7, 2026)

Before finalizing and submitting **Milestone 2**, **Harty Joy Villegas and Kimberly Joy Goyena** attended a mentoring session with the instructor on **March 7, 2026 at 8:45 AM**. During this session, the system implementation was presented, including the program code and the console-based execution of the MotorPH Payroll System.

The instructor reviewed the payroll logic, system output, and code structure and provided clarifications regarding several aspects of the implementation. Based on the feedback received during the mentoring session, **Harty Joy Villegas and Kimberly Joy Goyena conducted a follow-up coordination through Google Meet** to review the notes, apply the required fixes, and finalize the system implementation.

During this working session, both members collaborated in updating the code, validating payroll computations, performing additional testing, and completing the repository updates. The finalized version of the system was then **committed and pushed to the GitHub repository** before the Milestone 2 submission.

### Clarifications Confirmed by the Mentor

- **Employee Naming Convention**  
  The instructor confirmed that using **First Name** for employees is acceptable for the system implementation.

- **Salary Output with Decimal Values**  
  The instructor clarified that salary values may display **multiple decimal places**, and advised **not to round off values** to avoid inconsistencies in payroll calculations.

- **Currency Symbol (PHP)**  
  The instructor confirmed that including the **PHP currency symbol is not required**, so the system maintained the numeric format for salary outputs.

### Corrections and Improvements Implemented

- **Total Deductions Calculation**  
  The computation of **Total Deductions** was corrected to ensure that **withholding tax is included**, resolving the earlier inconsistency in deduction totals.

- **Code Cleanup**  
  Notes and unnecessary comments in the code were removed to improve readability.

- **Removal of Unused Methods and Variables**  
  Unused code elements (for example previously unused methods and variables) were removed to improve maintainability and code clarity.

- **Naming Convention Improvements**  
  Variable and method names were standardized using **camelCase** naming conventions.

- **Employee ID Input Handling**  
  Input validation was improved so that if a user enters invalid characters for the employee ID, the system displays an appropriate prompt message instead of causing the program to crash.

### Testing Validation

Additional testing was performed to validate the corrections made after the mentoring session. These tests confirmed that:

- Payroll computations produce consistent results
- Government deductions are applied correctly
- Input validation prevents system crashes
- The system runs properly in the console environment

These updates were applied prior to the final commit and push of the repository for **Milestone 2 submission**.
